...
router.post('/', async (req, res) => {
  try {
    ...
    // Validation
    // Dork Unit Price Jenh
    if (!productData.sku || !productData.productName ||
      !productData.boxPrice || !productData.ctnPrice) {
      return res.status(400).json({
        success: false,
        message: 'SKU, product name, and all prices are required'
      });
    }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (!productData.productName ||
      ...
      return res.status(400).json({
        success: false,
        message: 'Product name and all prices are required'
      });
    }
