...
const app = {
  ...
  addItemRow() {
    ...
    const productOptions = this.products.map(p =>
      `<option value="${p.id}" data-sku="${p.sku}" data-name="${p.product_name}" data-desc="${p.description || ''}"
        data-unit="${p.unit_price}" data-box="${p.box_price}" data-ctn="${p.ctn_price}">
    ...
    itemRow.innerHTML = `
      <div>
        <div>
          ...
          <select class="item-product" required onchange="app.onProductSelect(this)">
            <option value="">Select Product</option>
            ${productOptions}
          </select>
        </div>
        <div>
          <label style="font-size: 12px;">Unit Type / FOC</label>
          <div class="unit-type-buttons" style="display: flex; gap: 5px;">
              <!-- removed UNIT option per user request -->
            <button type="button" class="unit-btn" data-type="BOX" disabled onclick="app.selectUnitType(this, event)">BOX</button>
            <button type="button" class="unit-btn" data-type="CTN" disabled onclick="app.selectUnitType(this, event)">CTN</button>
            <button type="button" class="unit-btn foc-btn" data-type="FOC" disabled onclick="app.selectUnitType(this, event)" style="background: #ff9800;">FOC</button>
          </div>
          <div class="foc-unit-selector" style="display: none; margin-top: 5px;">
            <label style="font-size: 12px;">FOC as:</label>
            <select class="foc-unit-select" style="width: 100%; padding: 6px; font-size: 12px;" onchange="app.updateFOCUnit(this)">
              <option value="BOX">BOX</option>
              <option value="STREAMER">STREAMER</option>
            </select>
          </div>
          <input type="hidden" class="item-unit-type" value="">
          <input type="hidden" class="item-foc-unit" value="">
        </div>
        <div>
          <label style="font-size: 12px;">Price</label>
          <input type="number" class="item-price" placeholder="0.00" min="0" step="0.01" readonly>
        </div>
        <div>
          <label style="font-size: 12px;">Qty</label>
          <input type="number" placeholder="Qty" class="item-quantity" min="1" value="1" required onchange="app.updateTotal()" oninput="app.updateSubtotal(this.closest('.item-row'))">
        </div>
        <div>
          <label style="font-size: 12px;">Subtotal</label>
          <div class="item-subtotal" style='text-wrap: nowrap;'>$ 0.00</div>
        </div>
        <button type="button" class="btn-remove" onclick="app.removeItemRow(this)">×</button>
      </div>
    `;
  ...
  onProductSelect(selectElement) {
    ...
    if (!productId) {
      ...
      itemRow.querySelectorAll('.unit-btn').forEach(btn => btn.disabled = true);
      itemRow.querySelector('.item-price').value = '';
      itemRow.querySelector('.item-unit-type').value = '';
      return;
    }

    // Enable all unit type buttons (FOC is now always available)
  ...
  selectUnitType(button, event) {
    ...
    if (unitType === 'BOX') {
      ...
      if (!itemRow.querySelector('.item-foc-unit').value) {
        itemRow.querySelector('.item-foc-unit').value = itemRow.querySelector('.foc-unit-select').value || 'BOX';
      }
    }
  ...
  updateTotal() {
    ...
    document.getElementById('totalAmount').textContent = `$ ${total.toFixed(2)} = ៛ ${(total * 4000)}`;
  },
  ...
  async submitForm() {
    ...
    document.querySelectorAll('.item-row').forEach(row => {
      ...
      const originalUnit = row.querySelector('.item-foc-unit').value || unitType;
    ...
    if (items.length === 0) {
      this.showMessage('Please add at least one item with a unit type or FOC selected', 'error');
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        this.displayReceipt(result.invoiceId, result.invoiceNumber, invoiceData);
      } else {
  ...
  displayReceipt(invoiceId, invoiceNumber, invoiceData) {
    ...
    invoiceData.items.forEach(item => {
      ...
      let line;
      const unitDisplay = item.unitType === 'FOC' ? item.originalUnit : item.unitType;
